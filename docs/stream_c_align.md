...
## 현재 상태: T=5 QA 완료(Step 9 제외) — `background/api.ts` 코드 레벨 검증 끝

Step 7(`/quiz`)·Step 8(`/scrap`) 구현 + T4.9 버그 수정 + T=5 QA 하니스(`npm run qa:scrap`, 17/17)까지 전부 완료. 남은 Stream C 몫은 **Step 9(팝업 로그인, 담당2 스키마 대기 중 블로킹)**뿐.
...
Stream C 역할:
- [x] `/quiz`, `/scrap` 코드 레벨 end-to-end 확인 — **완료.** `npm run qa:scrap`(신규 하니스, `extension/qa/scrap-qa.ts`) 17/17 통과. 실제 `background/api.ts`·`mockQuiz.ts` 코드를 그대로 import해서(재구현 아님) FIFO 순서·중간실패·poison message 드롭·동시 drain(뮤텍스)·Bearer best-effort·mock anchorText 파생을 검증. 상세는 [stream_c_qa.md](./stream_c_qa.md).
- [ ] 팝업 로그인 → 401 재로그인 유도 플로우 확인 — **Step 9 완료 후에만 가능(현재 블로킹)**
- [x] 데모 기사 3~5개로 전체 플로우(A/B 포함) 통과 확인 — A의 `stream_a_qa.md`가 이미 담당(앵커 QA 16/16 + 브라우저 e2e 체크리스트). C 영역(quiz/scrap)은 위 하니스로 별도 커버했으므로 중복 작성 안 함.

**T=5 산출물:** [stream_c_qa.md](./stream_c_qa.md) 작성 완료 — 완성도, QA 하니스 결과, 튜닝 노브, Step 9 핸드오프, 알려진 한계 정리.
...
- **T=5** (2026-07-21): A의 `stream_a_qa.md` 검토(앵커 QA 16/16, 브라우저 e2e 체크리스트 — C 몫은 없음). Stream C 자체 QA로 `extension/qa/scrap-qa.ts`(`npm run qa:scrap`) 신설 — A의 `qa:anchor` 패턴 그대로, 재구현이 아니라 실제 `background/api.ts`·`mockQuiz.ts`를 import해서 `chrome.storage.local`·`fetch`만 스텁으로 교체. FIFO 순서·중간실패·poison 드롭(T4.9 finding #2)·동시 drain 무결성(T4.9 finding #1)·Bearer best-effort·mock anchorText 파생 6그룹 17/17 통과. `docs/stream_c_qa.md` 작성(완성도·QA 결과·튜닝 노브·Step 9 핸드오프·알려진 한계). Stream C의 T=5는 Step 9(담당2 블로킹) 제외 전부 완료.
