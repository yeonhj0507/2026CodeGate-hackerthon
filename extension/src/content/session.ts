...
//          정답 → 결과기록 + IDLE (다음 문단 대기)
//          오답 → 결과기록 + SHOW_EXPLANATION
//     → (설명 닫기: dismissExplanation)
...
export const useSession = create<Store>((set, get) => ({
   ...
   * 이미 다른 문항 진행 중(ASKING/SHOW_EXPLANATION)이면 무시 — 한 번에 한 문항만.
   */
  startQuestion: (quiz) => {
   ...
   * 정답이면 개념 이해로 보고 IDLE 복귀, 오답이면 설명 노출.
   */
  submitAnswer: (selectedIndex) => {
    ...
    set({
      ...
      phase: correct ? 'IDLE' : 'SHOW_EXPLANATION',
      active: correct ? null : active,
      parentConcept: correct ? null : parentConcept,
    })
  },

  /**
   * 오답 설명 확인 후 호출. 선행 개념 재질문이 있고 아직 2단계 미만이면 한 단계
   * 파고들고, 없으면 세션을 IDLE로 되돌린다.
   */
  dismissExplanation: () => {
    const { phase, active } = get()
    if (phase !== 'SHOW_EXPLANATION' || !active) return
