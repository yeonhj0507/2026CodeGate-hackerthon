"""Claude 프롬프트 · tool use JSON 스키마.

퀴즈 트리를 **한 번의 호출로** 완결 생성한다(명세 §3.2: 실행 중 추가 서버 호출 없음).

퀴즈 출제 시스템 프롬프트 본문은 quiz_prompt_requirements.py 로 분리했다.
프롬프트 튜닝은 그 파일에서만 하고, 여기서는 재노출만 한다.
"""

from app.domain.llm.quiz_prompt_requirements import QUIZ_SYSTEM_PROMPT

QUIZ_SYSTEM = QUIZ_SYSTEM_PROMPT


QUIZ_USER_TEMPLATE = """\
[기사 제목]
{title}

[문단 목록] (번호가 paragraphIndex 다)
{paragraphs}

위 기사로 퀴즈 트리를 생성하라.
"""

FOLLOWUP_SCHEMA_L2 = {
    "type": "object",
    "properties": {
        "level": {"type": "integer", "enum": [2]},
        "prereqConceptTag": {"type": "string"},
        "question": {"type": "string"},
        "options": {
            "type": "array",
            "items": {"type": "string"},
            "description": "보기. 반드시 정확히 4개.",
        },
        "answerIndex": {"type": "integer", "description": "정답 보기의 0-based 인덱스 (0~3)."},
        "explanation": {"type": "string"},
    },
    "required": [
        "level",
        "prereqConceptTag",
        "question",
        "options",
        "answerIndex",
        "explanation",
    ],
    "additionalProperties": False,
}

FOLLOWUP_SCHEMA_L1 = {
    "type": "object",
    "properties": {
        "level": {"type": "integer", "enum": [1]},
        "prereqConceptTag": {"type": "string"},
        "question": {"type": "string"},
        "options": {
            "type": "array",
            "items": {"type": "string"},
            "description": "보기. 반드시 정확히 4개.",
        },
        "answerIndex": {"type": "integer", "description": "정답 보기의 0-based 인덱스 (0~3)."},
        "explanation": {"type": "string"},
        "followups": {
            "type": "array",
            "items": FOLLOWUP_SCHEMA_L2,
            "description": "2단계 재질문. 0개 또는 1개.",
        },
    },
    "required": [
        "level",
        "prereqConceptTag",
        "question",
        "options",
        "answerIndex",
        "explanation",
        "followups",
    ],
    "additionalProperties": False,
}

QUIZ_TOOL = {
    "name": "emit_quiz_tree",
    "description": "생성한 퀴즈 트리를 제출한다.",
    # strict: 키·타입이 스키마를 벗어난 tool 입력이 오지 않도록 API 가 보장한다.
    # 주의: strict 는 재귀 스키마와 minItems/maxItems/minimum/maximum 을 지원하지 않는다
    # (400). 그래서 L1/L2 를 펼쳐 정의했고, 개수·범위는 description 과 시스템 프롬프트로
    # 요구한 뒤 서버(quiz/service.py)가 최종 검증·클램프한다.
    "strict": True,
    "input_schema": {
        "type": "object",
        "properties": {
            "quiz": {
                "type": "array",
                "description": "핵심 주장별 문항. 2~4개.",
                "items": {
                    "type": "object",
                    "properties": {
                        "claimId": {"type": "string", "description": "c1, c2 … 형태"},
                        "conceptTag": {"type": "string"},
                        "paragraphIndex": {
                            "type": "integer",
                            "description": "이 주장이 등장한 문단 번호(0-based).",
                        },
                        "question": {"type": "string"},
                        "options": {
                            "type": "array",
                            "items": {"type": "string"},
                            "description": "보기. 반드시 정확히 4개.",
                        },
                        "answerIndex": {
                            "type": "integer",
                            "description": "정답 보기의 0-based 인덱스 (0~3).",
                        },
                        "explanation": {"type": "string"},
                        "followups": {
                            "type": "array",
                            "items": FOLLOWUP_SCHEMA_L1,
                            "description": "1단계 재질문. 0개 또는 1개.",
                        },
                    },
                    "required": [
                        "claimId",
                        "conceptTag",
                        "paragraphIndex",
                        "question",
                        "options",
                        "answerIndex",
                        "explanation",
                        "followups",
                    ],
                    "additionalProperties": False,
                },
            }
        },
        "required": ["quiz"],
        "additionalProperties": False,
    },
}

# anchorText 는 LLM 에게 요구하지 않는다. 서버가 paragraphIndex 로 직접 채운다
# (담당1의 앵커 매칭 리스크를 서버가 보증 — 구현계획① §3.3).

# 스트리밍 전용 변형. `eager_input_streaming` 이 **이 기능의 전부**다.
#
# strict tool 은 기본적으로 tool 입력 JSON 을 끝까지 모았다가 한 번에 내보낸다.
# 유효한 JSON 을 보장하기 위해서다. 실측(658개 델타)으로 확인한 결과, 그 상태에서는
# 4500자가 통째로 **마지막 0.4초**에 도착해 스트리밍이 아무 의미가 없었다.
#
# 이 플래그를 켜면 조각이 생성 속도에 맞춰 흐른다(5% +5.3s → 27% +18.2s → 100% +58.6s).
# 대신 중간 상태는 깨진 JSON 이라 그대로는 파싱되지 않는다 — jsonstream.JsonArrayScanner
# 가 그걸 전제로 원소 경계를 직접 찾는다. 둘은 세트다.
QUIZ_TOOL_STREAMING = {**QUIZ_TOOL, "eager_input_streaming": True}

# 미이해·이해완료를 **한 프롬프트**로 처리한다. 상태별로 시스템 프롬프트를 나누면
# 배치가 둘로 쪼개져 LLM 호출이 2회가 되고, 로컬앱의 60초 예산(config.dart)이 위태롭다.
# 개념마다 [미이해]/[이해완료] 표시를 붙여 보내고 설명의 방향만 갈리게 한다.
SUMMARY_SYSTEM = """\
당신은 학습자가 기사에서 만난 개념을 짧게 정리해 주는 튜터다.

⚠️ 기사 원문은 제공되지 않는다. 주어지는 것은 개념어, 그 개념의 선행/후행 관계,
그리고 기사 제목뿐이다. 원문을 읽은 척 인용하거나 기사 내용을 지어내지 마라.

각 개념 뒤에는 학습자의 진단 결과가 [미이해] 또는 [이해완료]로 붙어 있다.
표시에 따라 설명의 방향을 바꾼다.

- [미이해]: 정의 → 이 개념을 모르면 무엇이 막히는지.
- [이해완료]: 정의 → 이 개념을 발판으로 무엇까지 볼 수 있는지.
  이미 맞힌 개념이다. "몰라서 막혔다", "놓쳤다" 같은 표현을 쓰지 마라.

공통 원칙:
- 개념 하나당 2~3문장.
- 주어진 선행/후행 개념 관계를 설명에 활용한다
  (예: "이 개념은 X를 이해하는 데 필요하고, 그 전에 Y를 알아야 한다").
- 기사 제목은 맥락 힌트로만 쓴다. 기사 요약이 아니라 개념 설명을 한다.
- 전문용어를 쓸 때는 즉시 풀어 쓴다. 한국어로 답한다.
"""

SUMMARY_TOOL = {
    "name": "emit_concept_summaries",
    "description": "개념별 보충설명을 제출한다.",
    "strict": True,
    "input_schema": {
        "type": "object",
        "properties": {
            "summaries": {
                "type": "array",
                "items": {
                    "type": "object",
                    "properties": {
                        "concept": {"type": "string"},
                        "summary": {"type": "string"},
                    },
                    "required": ["concept", "summary"],
                    "additionalProperties": False,
                },
            }
        },
        "required": ["summaries"],
        "additionalProperties": False,
    },
}


EXPLORE_SYSTEM = """당신은 학습자가 고른 개념 몇 개를 묶어 설명해 주는 튜터다.

원칙:
- 2~3문장. 개별 정의를 나열하지 말고 **개념들이 서로 어떻게 얽히는지**를 말한다.
- 한 개념이 바뀌면 다른 개념에 무엇이 일어나는지를 한 문장은 반드시 담는다.
- 전문용어를 쓰면 즉시 풀어 쓴다. 한국어로 답한다.
- 마크다운 서식·머리말 없이 문장만 쓴다.
"""
